...
export class ConnectionManager extends EventEmitter {
  ...
  private handlePeerUpdate({ detail: { peer } }: { detail: { peer: { id: PeerId, addresses: Array<{ multiaddr: any }> } } }): void {
    const multiaddrs = peer.addresses.map(({ multiaddr }) => multiaddr);
    log(`Changed multiaddrs: peer ${peer.id.toString()} multiaddrs: ${multiaddrs}`);
    this.emit('peer:update', { peer });
  }

  private handlePeerDiscovery(event: { detail: { multiaddrs: any[], id: PeerId } }): void {
